/*
     ...
     $Id: fntLocal.h,v 1.1 2010/10/25 09:15:02 jcl Exp $
*/
...
#else
  //(*QDIN) >> x;
  QDIN->read((char *)&x,sizeof(unsigned char ));
#endif
  return x ;
}
...
#else
  //(*QDIN) >> x;
  QDIN->read((char *)&x,sizeof(unsigned short ));
#endif
  _fnt_swab_short ( & x ) ;
  return x ;
}
...
#else
  //(*QDIN) >> x;
  QDIN->read((char *)&x,sizeof(unsigned int ));
#endif
  _fnt_swab_int ( & x ) ;
  return x ;
}
